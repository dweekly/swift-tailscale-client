...
- **Upstream request/version/error contract** (upstream-readiness issue 03):
  ...
  - New typed errors: `.permissionDenied(body:endpoint:)` (HTTP 403), `.rateLimited(retryAfterSeconds:body:endpoint:)` (HTTP 429; `Retry-After` parsed as delta-seconds or HTTP-date, malformed/negative/non-finite values yield `nil` instead of nonsense), and `.peerNotFound(endpoint:)` (`whois` 404, upstream `ErrPeerNotFound`).
...
- **Documentation as a product** (post-review overhaul):
  ...
  - `Scripts/check-release-consistency.sh`: CI and the release workflow fail when README, DocC, the agent skill, coverage metadata, CLAUDE.md, CHANGELOG, and the release tag disagree on the version — and now also when any doc disagrees with the source on the default capability version.
