...
- **Stable-parity gap-fill** (upstream-readiness issue 04, all but the DialTCP/UserDial design spike): typed whois variants — `whois(address:protocol:)`, `whois(nodeKey:)`, `whois(address:scopedToDestination:)`, `whois(address:forService:)` — sharing the typed `.peerNotFound` on 404; `checkUpdate()` over `update/check` (reports only, installs nothing; `ClientVersionStatus` enriched with latest-version/urgency/notify fields); `disconnectControl()` with explicit administrative documentation (HA replica drain — never exercised against live daemons in tests); `checkUDPGROForwarding()` (upstream annotates this one *unstable* — shipped as supported normalization, not a stable-parity item); and a supported `bugReport(note:)` facade (the diagnose/record knobs stay in `experimental`). Upstream `SwitchToEmptyProfile` needs no new API: `addProfile()` already performs the identical `PUT /profiles/` and now documents the equivalence, with the daemon's `201 Created` contract pinned by test. The `upstream_stable_unimplemented` ledger is down to `DialTCP`/`UserDial`.
