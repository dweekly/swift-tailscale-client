...
- Coverage: the CI gate surfaces the measured percentage as a check-run annotation (plus the ten worst-covered files), and a new edge-coverage suite exercises every error case's description/recovery/preview text, model encode paths, reconnect backoff growth, and redaction edge shapes — groundwork for the 85% floor ratchet.
- **`StartOptions.updatePrefs`** — `start(options:)` can now carry upstream `ipn.Options.UpdatePrefs`: preferences that replace the stored ones when the backend starts. This is the missing piece of the auth lifecycle: `logout()` deletes the profile (control URL included), so a subsequent `loginInteractive()` dialed the *default* control plane — observed live in the hermetic lane, where headscale could never approve a `login.tailscale.com` URL. Replacement semantics (nil fields become zero values, not defaults) are documented on the property.
- **Scripted login-lifecycle test** (the one open item carried from v0.9.0): the hermetic headscale lane now drives the interactive auth flow end to end — `logout()` → re-seed the control URL via `start(options:)` → `loginInteractive()` → `BrowseToURL` from the IPN bus (asserted to point at the seeded control server) → approval via the headscale CLI → Running. Triple-gated (`TAILSCALE_INTEGRATION_LOGIN=1` on top of the integration/write gates) and run as a separate final step so the main suite always sees a logged-in daemon.
